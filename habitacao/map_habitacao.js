...
function handleCouncilClick(council, sliderValue = null) {
    ...
    if (datavar.includes("Habitantes")) {
    ...
    } else if (datavar.includes("gdg")) {
        dataFile = councilData_habitacoes;
		findCouncil(dataFile, council, datavar, datayear)
		...
		document.getElementById("suboption").innerHTML = null;
		document.getElementById('radio-menu').innerHTML = null;
    }else if (datavar.includes("Preço")) {
		dataFile = councilData_habitacoes;
		findCouncil(dataFile, council, datavar, datayear);
		const anos = Object.keys(dataFile[council][datavar]).sort();
		...
		createRadioMenu(dataFile, council, datavar, datayear)
		colorCouncilByData(dataFile, datavar, datayear, "#FF0000", "#15a708", councilexcl, radioselection);
		createCouncilTimeline(dataFile, council, datavar, anos.slice(1), false, false, radioselection);
		createCouncilHistogram(dataFile, datavar, datayear, radioselection, councilexcl);
        document.getElementById("pyramid-container").innerHTML = null;
	}
    ...
    for (const [councilName, data] of Object.entries(dataFile)) {
        if (councilName === council) {
            ...
            if (data[dataVariable] && data[dataVariable][year]) {
				...
				if (['Habitantes', 'dg','Imigração','Estrangeira'].includes(dataVariable)) {
					if (councilInfo) {
				...
				} else if (dataVariable === 'Preço') {
					if (councilInfo) {
						councilInfo.innerHTML = `
							${dataVariable.charAt(0).toUpperCase() + dataVariable.slice(1)} por metro2 em euros
							<p> Total (${year}): ${data[dataVariable][year]["Total"].toFixed(2)}</p>
							<p> variação (${year}): ${data[dataVariable][year]["variação"].toFixed(2)}</p>
						`;
					}
				} else {
					return {};
...
function createCouncilHistogram(dataFile, dataVariable, year, subvariable, excludedCouncils = []) {
    // Input validation
    ...
    // Get values from included councils for the specified year
    const values = Object.values(includedCouncils)
        .map(data => data[dataVariable][year][subvariable]);
    ...
    // Draw axes
    // ctx.beginPath();
    //ctx.moveTo(25, canvas.height - 55);
    //ctx.lineTo(25, 25);
    ...
    bins.forEach((count, index) => {
        ...
        const y = canvas.height - 55 - height;
...
function createCouncilTimeline(dataFile, council, dataVariable, years, showHomem = true, showMulher = true,varselect) {
    ...
    // Add title
    ctx.textAlign = 'center';
    ctx.fillText(`${council} - ${dataVariable} ${categ} por Ano`, canvas.width / 2, 30);
...
function createBarChart(dataFile, dataVariable, year, council) {
    ...
    const categories = Object.entries(yearData)
		  .filter(([key]) => !['variação', 'variação_apartamento','variação_moradia'].includes(key))
		  .map(([category, data]) => ({
			category,
            value: data
        }));
    ...
    ctx.fillText(`${council} - ${dataVariable} (${year})`, canvas.width/2, padding/2);
    ctx.restore();
